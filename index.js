...
let raspisanie = [['9:30-11:10 ОПЕРАЦИОННЫЕ СИСТЕМЫ – ПРАКТ., АУД. каф.САПРиУ Разыграев', '11:30-13:10 МОДЕЛИРОВАНИЕ СИСТЕМ — ЛАБОР., ПРАКТ., Холоднов В.А. АУД. каф.', '14:00-15:40 ТЕОРЕТИЧЕСКИЕ ОСНОВЫ ХИМИЧЕСКОЙ ТЕХНОЛОГИИ –ЛЕКЦИЯ, АУД. 206 Черемисина О.А.', '16:00-17:40 УПРАВЛЕНИЕ ПРОЕКТАМИ РАЗРАБОТКИ ПРОГРАММНОГО ОБЕСПЕЧЕНИЯ И СИСТЕМНАЯ АНАЛИТИКА ПРИКЛАДНОЙ ОБЛАСТИ -ЛЕКЦИЯ, ПРАКТ, АУД. КАФ. доц. Мамаева Г.А.'],
...
let raspisanieN = [['9:30-11:10 ', '11:30-13:10', '14:00-15:40 ', '16:00 - 17:40 '],
...
const start = () => {
    ...
    bot.on('callback_query', async msg => {
        ...
        if (week === '/even') {
            if (data === '/monday') {
                for (let i = 0; i < raspisanie[0].length; i++) {
                    await bot.sendMessage(chatId, raspisanie[0][i])
                }
            }

            if (data === '/tuesday') {
                for (let i = 0; i < raspisanie[1].length; i++) {
                    await bot.sendMessage(chatId, raspisanie[0][i])
                }
            }

            if (data === '/wednesday') {
                for (let i = 0; i < raspisanie[2].length; i++) {
                    await bot.sendMessage(chatId, raspisanie[0][i])
                }
            }

            if (data === '/thursday') {
                for (let i = 0; i < raspisanie[3].length; i++) {
                    await bot.sendMessage(chatId, raspisanie[0][i])
                }
            }

            if (data === '/friday') {
                for (let i = 0; i < raspisanie[4].length; i++) {
                    await bot.sendMessage(chatId, raspisanie[0][i])
                }
            }
        }
